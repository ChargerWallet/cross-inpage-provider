...
class ProviderTron extends ProviderTronBase implements IProviderTron {
  ...
  private _registerTronWeb(nodes: Nodes): { tronWeb: TronWeb; sunWeb: SunWeb } | null {
    ...
    const tronWeb: TronWeb = new ChargerWalletTronWeb(
      {
        ...nodes,
      },
      this,
    ...
    const tronWeb1: TronWeb = new ChargerWalletTronWeb(
      {
        ...nodes,
      },
      this,
    ...
    const tronWeb2: TronWeb = new ChargerWalletTronWeb(
      {
        ...nodes,
      },
      this,
  ...
  private async _initialize() {
    try {
      ...
      if (window.tronWeb !== undefined) {
        this._log.warn(
          'ChargerWallet: TronWeb is already initiated. ChargerWallet will overwrite the current instance',
        );
      }
      ...
      Object.defineProperty(tronWeb, 'defaultAddress', {
        get() {
          if (!self._connected) {
            self._log.warn(
              'ChargerWallet: We recommend that DApp developers use $chargerwallet.tron.request({method: "tron_requestAccounts"}) to request users’ account information at the earliest time possible in order to get a complete TronWeb injection.',
            );
  ...
  private _handleAccountsChanged(accounts: string[]) {
    ...
    if (!Array.isArray(accounts)) {
      this._log.error(
        'ChargerWallet: Received invalid accounts parameter. Please report this bug.',
        accounts,
      );
      _accounts = [];
    }

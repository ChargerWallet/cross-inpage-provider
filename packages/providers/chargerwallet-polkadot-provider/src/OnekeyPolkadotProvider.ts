...
class ProviderPolkadot extends ProviderPolkadotBase implements IProviderPolkadot {
  ...
  private createMessage(payload: IMessagePayload): IPostMessage {
    return {
      id: 2,
      origin: 'Charger Wallet Polkadot Provider',
      ...payload,
    };
  }
...
const registerPolkadot = (provider: ProviderPolkadot, name = 'Charger Wallet', version = '1.0.0') => {
  try {
    ...
    injectExtension(enableFn, { name: name ?? 'Charger Wallet', version: version ?? '1.0.0' });
  } catch (error) {
    console.error(error);
  }
};

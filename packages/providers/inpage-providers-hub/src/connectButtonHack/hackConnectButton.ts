...
import type { IWindowChargerWalletHub } from '../injectWeb3Provider';
...
function checkIfInjectedProviderConnected({
  ...
  const hub = window.$chargerwallet as IWindowChargerWalletHub;
...
export async function detectQrcodeFromSvg({
  ...
  const res = (await (window.$chargerwallet as IWindowChargerWalletHub)?.$private?.request({
...
export function createWalletConnectToButton({
  ...
  const chargerwalletHub = window.$chargerwallet as IWindowChargerWalletHub | undefined;
...
text-align: center;
    ...
    btn.onclick = () => {
      ...
      void chargerwalletHub?.$private?.request({
        method: 'wallet_connectToWalletConnect',
        params: { uri },
      });
      const spinner = btn.querySelector('.chargerwallet-spinner-element') as HTMLElement | undefined;
      if (spinner) {

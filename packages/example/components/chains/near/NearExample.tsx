...
} from '@chargerwallet/chargerwallet-near-provider';
...
declare global {
  interface Window {
    provider: ChargerWalletNearProvider;
    nearAPI: typeof NearApi;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ChargerWalletNearProvider: any;
  }
}

window.nearAPI = NearApi;
window.ChargerWalletNearProvider = ChargerWalletNearProvider;
...
export default function NearExample() {
  const [provider, setProvider] = useState<ChargerWalletNearProvider | null>(null);
  ...
  useEffect(() => {
    ...
    const _provider = new ChargerWalletNearProvider({
      // connection,
  ...
  return (
    <div>
      ...
      {!provider && (
        <a target="_blank" href={'https://www.chargerwallet.com/download/'}>
          Install ChargerWallet Extension →
        </a>
      )}
      {provider && (
        <div>
            ...
            <button
              onClick={async () => {
                const res = (await provider.request({
                  ...
                  params: {
                    messages: ['hello world', 'chargerwallet near wallet'],
                  },
                })) as SignMessagesResult;

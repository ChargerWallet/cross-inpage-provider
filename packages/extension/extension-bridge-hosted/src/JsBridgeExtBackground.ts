...
const { EXT_PORT_OFFSCREEN_TO_BG, EXT_PORT_CS_TO_BG, EXT_PORT_UI_TO_BG, CHARGERWALLET_ALIGN_PRIMARY_ACCOUNT } = consts;
  ...
  requestToAllCS(scope: IInjectedProviderNamesStrings, data: unknown, targetOrigin?: string) {
    ...
    entries(this.ports).forEach(async ([portId, port]) => {
      if (port.name === EXT_PORT_CS_TO_BG) {
        ...
        if (!targetOrigin || targetOrigin === origin || targetOrigin === CHARGERWALLET_ALIGN_PRIMARY_ACCOUNT) {

...
import { IInpageProviderConfig, ProviderBase, switchDefaultWalletNotification, switchNetworkNotification } from '@chargerwallet/cross-inpage-provider-core';
...
class ProviderPrivate extends ProviderBase {
        ...
        if (method === METHODS.wallet_events_ext_switch_changed) {
          ...
          if (!walletInfoLocal || (walletInfoLocal && walletInfoLocal.platformEnv.isExtension)) {
            try {
              ...
              this.notifyDefaultWalletChanged(params as IChargerWalletWalletInfo)
            } catch (e) {
              console.error(e);
            }
          }
  ...
  notifyDefaultWalletChanged(params: IChargerWalletWalletInfo) {
